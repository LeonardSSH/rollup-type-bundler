import { packageCwd, rollupTypeBundlerRcJsonPath, rollupTypeBundlerRcPath, rollupTypeBundlerRcYamlPath, rollupTypeBundlerRcYmlPath } from '#lib/constants';
...
export async function parseOptionsFile(cliOptions) {
    ...
    if (rollupTypeBundlerRcYamlExists || rollupTypeBundlerRcYmlExists) {
        try {
            const fileOptions = await readYaml(rollupTypeBundlerRcYamlExists ? rollupTypeBundlerRcYamlPath : rollupTypeBundlerRcYmlPath);
            options = {
                ...fileOptions,
                ...options,
        ...
        try {
            const fileOptions = await readJson(rollupTypeBundlerRcExists ? rollupTypeBundlerRcPath : rollupTypeBundlerRcJsonPath);
            options = {
                ...fileOptions,
                ...options,
                external: [...(fileOptions.external ?? []), ...(options.external ?? [])]
 ...
 */
function transformOptionsDistPathToFileUrl(options) {
